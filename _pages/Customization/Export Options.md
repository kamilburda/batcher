...
* *Layer handling*: If set to `Keep layers` and the image has multiple layers, they are kept intact. If set to `Merge and add alpha`, layers are merged into a single rasterized layer with an alpha channel. Any filters (layer effects) are also merged. This avoids surprising results as some filters may expand the image canvas. If exporting to a file format supporting layers or filters (such as XCF or PSD) or to a vector graphics format (such as SVG) and you want to preserve these features, use `Keep layers`. `Merge and remove alpha` works like `Merge and add alpha`, except it also removes the alpha channel and applies the specified background color.
