...
* *Merge visible layers and rasterize*: If checked, layers in each image are merged into a single rasterized layer. Any filters (layer effects) are also merged. Vector layers are rasterized. This avoids surprising results as some filters may expand the image canvas. If you intend to export using a file format supporting layers and filters (such as XCF or PSD) or to a vector graphics format (such as SVG) and you want to preserve the layers/filters/vector graphics, uncheck this option.
